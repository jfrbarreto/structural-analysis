...
interface LocalStateBlockProps {
  handleLocalStateInput(e: ChangeEvent<HTMLInputElement>): void;
  localStateInputValue: string;
}

export const LocalStateBlock: React.FC<LocalStateBlockProps> = (props) => {
  const { handleLocalStateInput, localStateInputValue } = props;
...
export const StoreBlock: React.FC = (props) => {
  const { handleStoreStateInput, storeStateInputValue } = useSharedHelper();
